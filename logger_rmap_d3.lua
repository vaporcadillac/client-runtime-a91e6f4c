-- Remote Map Dump Logger | Volt autoexec | Luau
-- Run: drop in Volt autoexec on ONE account farming with GScript.
--      Farm one normal session (20-30 min), then remove the file.
-- Out: remote_dump_<userId>.json + remote_dump_<userId>.log in workspace.
-- Passive: hooks __namecall/queue on Instance only. Never touches
...
local SESSION_SECONDS = 60 * 60 -- hard stop; one session is plenty

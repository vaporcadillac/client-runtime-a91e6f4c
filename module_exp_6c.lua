local env = getgenv()
local ENGINE_BUILD = "experimental-hyperflow-6.1c"

local function startFleetLedger()
    if env.GLEDGER_ENABLED == false or env.__FLEET_PROFIT_LEDGER_RUNNING or env.__FLEET_PROFIT_LEDGER_LOADING then
        return
    end
    env.__FLEET_PROFIT_LEDGER_LOADING = true
    if env.GLEDGER_ROLE == nil then
        env.GLEDGER_ROLE = env.GPINATA_ENABLED == false and "GScript Account" or "Pinata Farmer"
    end
    task.spawn(function()
        local ledgerUrl = "https://raw.githubusercontent.com/vaporcadillac/client-runtime-a91e6f4c/main/module_f0a9.lua"
        local lastError = "download unavailable"
        for attempt = 1, 5 do
            if env.STOP_FLEET_PROFIT_LEDGER then break end
            local requestUrl = ledgerUrl .. "?session=" .. tostring(os.time()) .. "&attempt=" .. tostring(attempt)
            local downloadOk, source = pcall(function() return game:HttpGet(requestUrl) end)
            if downloadOk and type(source) == "string" and #source > 0 then
                ...
                if chunk then
                    ...
                    if not runtimeOk then warn("[LedgerBootstrap] Startup failed: " .. tostring(runtimeError)) end
                    return
                end
            ...
            if attempt < 5 then task.wait(math.min(20, attempt * 3)) end
        end
        env.__FLEET_PROFIT_LEDGER_LOADING = false
...
if env.GPINATA_ENABLED == false then return end
...
task.spawn(function()
    ...
    local function normalizeWebhookUrl(value)
        ...
        local url = value:match("%((https?://[^%)]+)%)") or value:match("(https?://%S+)")
        if not url then return nil end
        url = url:gsub("[>%)]+$", "")
        local lowerUrl = string.lower(url)
        local isDiscord = string.find(lowerUrl, "PASTE_YOUR_DISCORD_WEBHOOK_HERE", 1, true) == 1
            or string.find(lowerUrl, "https://discordapp.com/api/webhooks/", 1, true) == 1
            or string.find(lowerUrl, "https://canary.discord.com/api/webhooks/", 1, true) == 1
        return isDiscord and url or nil
    end

    local function resolveRequestFunction()
        local candidates = {}
        local function addCandidate(c) if type(c) == "function" then table.insert(candidates, c) end end
        ...
        for _, c in ipairs(candidates) do if type(c) == "function" then return c end end
        return nil
    end

    local webhookEnabled = env.GPINATA_WEBHOOK_ENABLED == true
    local webhookUrl = normalizeWebhookUrl(env.GPINATA_WEBHOOK_URL or env.GWEBHOOK_LINK)
    local webhookRequest = resolveRequestFunction()
    local webhookQueueLimit = math.max(5, math.min(100, math.floor(numberSetting("GPINATA_WEBHOOK_QUEUE_LIMIT", 30))))
    local webhookQueue = {}
    ...
    local function safeWebhookText(value, limit)
        ...
        if #text > limit then return string.sub(text, 1, math.max(1, limit - 3)) .. "..." end
        return text
    end
    ...
    local function formatDuration(seconds)
        ...
        if days > 0 then return string.format("%dd %02dh %02dm %02ds", days, hours, minutes, remainingSeconds) end
    ...
    local function formatCompact(value)
        ...
        if not number then return "Loading…" end
        local absolute = math.abs(number)
        local suffixes = { {1e15, "Q"}, {1e12, "T"}, {1e9, "B"}, {1e6, "M"}, {1e3, "K"} }
        for _, entry in ipairs(suffixes) do
            if absolute >= entry[1] then
                local compact = number / entry[1]
                local decimals = math.abs(compact) >= 100 and 0 or math.abs(compact) >= 10 and 1 or 2
    ...
    local function dashboardDescription(detail, targetZone)
        local stateIcons = {
            Running = "🟢", Calibrating = "🟡", Paused = "🟠", Circuit = "🔴",
            Fault = "🔴", Stopped = "⚫", Booting = "🔵",
        }
        local state = dashboard.state or "Booting"
        local stateIcon = stateIcons[state] or "🔵"
        local pinataText = dashboard.pinatas == nil and "Loading…" or formatInteger(dashboard.pinatas)
        local gemText = dashboard.gems == nil and "Loading…" or string.format("%s (`%s`)", formatCompact(dashboard.gems), formatInteger(dashboard.gems))
        local hourly = tonumber(dashboard.hourly) or 0
        local daily = tonumber(dashboard.daily) or 0
        local lines = {
            string.format("%s **%s**  •  Interval `%.2fs`", stateIcon, state, tonumber(dashboard.interval) or 0),
            string.format("💎 **%s gems**  •  🪅 **%s piñatas**", gemText, pinataText),
            string.format("⚡ **%.1f/hour**  •  **%.0f/day**  •  ✅ **%.2f%% success**", hourly, daily, tonumber(dashboard.successRate) or 0),
            string.format("🧠 **%s**  •  🔗 **%s**", tostring(dashboard.calibration or "Not available"), tostring(dashboard.remote or "Resolving")),
        }
        if type(detail) == "string" and detail ~= "" then table.insert(lines, "\n" .. detail) end
        return table.concat(lines, "\n")
    end

    local function webhookField(name, value, inline)
        return { name = safeWebhookText(name, 256), value = safeWebhookText(value, 1024), inline = inline == true }
    end
    ...
    local function appendWebhookFields(destination, source)
        for _, field in ipairs(source or {}) do
            if #destination >= 25 then break end
    ...
    local function readRetryAfter(response)
        if type(response) ~= "table" or type(response.Body) ~= "string" then return 1 end
        local ok, decoded = pcall(HttpService.JSONDecode, HttpService, response.Body)
        local retryAfter = ok and type(decoded) == "table" and tonumber(decoded.retry_after) or nil
    ...
    local function deliverWebhook(payload)
        ...
        if not encodedOk then return false, "JSON encoding failed" end
        for attempt = 1, 2 do
            local requestOk, response = pcall(webhookRequest, {
                Url = webhookUrl, Method = "POST", Headers = {["Content-Type"] = "application/json"}, Body = body,
            })
            if requestOk then
    ...
    startWebhookWorker = function()
        if not webhookEnabled or webhookWorkerRunning then return end
        ...
        task.spawn(function()
            while #webhookQueue > 0 do
                ...
                if not delivered and not webhookFailureWarned then
                    ...
                    warn("[Runtime] Pinata webhook delivery failed: " .. tostring(deliveryError))
                elseif delivered then
                    webhookFailureWarned = false
                end
                if #webhookQueue > 0 then task.wait(0.5) end
            end
            webhookWorkerRunning = false
            if #webhookQueue > 0 then startWebhookWorker() end
        end)
    end
    ...
    local function sendWebhook(options)
        ...
        options = options or {}
        if options.periodic ~= true then return end
        local fields = {}
        ...
        local shouldMention = options.critical == true and env.GPINATA_WEBHOOK_MENTION_ON_CRITICAL == true and mentionUserId ~= nil
        local payload = {
            username = safeWebhookText("Mini Pinata Monitor - " .. accountName, 80),
            content = shouldMention and ("<@" .. mentionUserId .. ">") or nil,
            allowed_mentions = shouldMention and { users = { mentionUserId } } or { parse = {} },
            embeds = {
                {
                    title = safeWebhookText(options.title or "Mini Pinata Update", 256),
                    description = safeWebhookText(dashboardDescription(options.description, options.targetZone), 4096),
                    color = tonumber(options.color) or 3447003,
                    fields = fields,
                    footer = { text = "Engine " .. ENGINE_BUILD },
                    timestamp = os.date("!%Y-%m-%dT%H:%M:%SZ"),
                },
            },
        }
        if #webhookQueue >= webhookQueueLimit then table.remove(webhookQueue, 1) end
        table.insert(webhookQueue, payload)
        startWebhookWorker()
    end

    local watchdogTimeout = math.max(20, numberSetting("GPINATA_WATCHDOG_TIMEOUT", 45))
    ...
    local function heartbeat(state)
        ...
        if state then heartbeatState = state end
    end

    local function cancelManagedThread(thread)
        local cancelled = false
        if type(task.cancel) == "function" then cancelled = pcall(task.cancel, thread) end
        if not cancelled and type(coroutine.close) == "function" then cancelled = pcall(coroutine.close, thread) end
        return cancelled
    end

    print(string.format("[Runtime] Engine build %s | watchdog %.0fs", ENGINE_BUILD, watchdogTimeout))
    ...
    local function runEngine()
        ...
        local SETTINGS = {
            -- OPTION C: Conservative 6.1s Probe
            -- Floor set to 6.1s. Max set to 6.5s to keep exploration tight.
            INTERVAL = math.max(1.5, numberSetting("GPINATA_INTERVAL", 6.1)),
            ADAPTIVE = env.GPINATA_ADAPTIVE ~= false,
            ADAPTIVE_MIN_INTERVAL = math.max(1.5, numberSetting("GPINATA_ADAPTIVE_MIN_INTERVAL", 6.1)),
            ADAPTIVE_MAX_INTERVAL = math.max(1.5, numberSetting("GPINATA_ADAPTIVE_MAX_INTERVAL", 6.5)),
            ADAPTIVE_WINDOW = math.max(10, math.floor(numberSetting("GPINATA_ADAPTIVE_WINDOW", 20))),
            ADAPTIVE_LONG_WINDOW = math.max(300, math.floor(numberSetting("GPINATA_ADAPTIVE_LONG_WINDOW", 300))),
            ADAPTIVE_STEP_DOWN = math.max(0.01, numberSetting("GPINATA_ADAPTIVE_STEP_DOWN", 0.05)),
            ADAPTIVE_HIGH_REJECT_RATE = math.max(0, numberSetting("GPINATA_ADAPTIVE_HIGH_REJECT_RATE", 0.25)),
            ADAPTIVE_PRESSURE_WINDOWS = math.max(1, math.floor(numberSetting("GPINATA_ADAPTIVE_PRESSURE_WINDOWS", 2))),
            ADAPTIVE_PRESSURE_STEP = math.max(0.01, numberSetting("GPINATA_ADAPTIVE_PRESSURE_STEP", 0.10)),
            ADAPTIVE_RECOVERY_HOLD = math.max(2700, numberSetting("GPINATA_ADAPTIVE_RECOVERY_HOLD", 2700)),
            ADAPTIVE_FAILURE_HOLD = math.max(3600, numberSetting("GPINATA_ADAPTIVE_FAILURE_HOLD", 3600)),
            ADAPTIVE_RECOVERY_MAX_REJECT_RATE = math.min(0.015, math.max(0, numberSetting("GPINATA_ADAPTIVE_RECOVERY_MAX_REJECT_RATE", 0.015))),
            ADAPTIVE_RECOVERY_MAX_SECOND_RETRIES = math.max(0, math.floor(numberSetting("GPINATA_ADAPTIVE_RECOVERY_MAX_SECOND_RETRIES", 0))),
            ISOLATED_FAILURE_WINDOW = math.max(50, math.floor(numberSetting("GPINATA_ISOLATED_FAILURE_WINDOW", 100))),
            ISOLATED_FAILURES_TO_SLOW = math.max(2, math.floor(numberSetting("GPINATA_ISOLATED_FAILURES_TO_SLOW", 2))),
            ISOLATED_FAILURE_STEP = math.max(0.05, numberSetting("GPINATA_ISOLATED_FAILURE_STEP", 0.10)),
            ...
            RECOVERY_RETRY_DELAY = math.max(0.5, numberSetting("GPINATA_RECOVERY_RETRY_DELAY", 2)),
            MAX_RETRIES = math.min(2, math.max(0, math.floor(numberSetting("GPINATA_MAX_RETRIES", 2)))),
            ...
            WEBHOOK_STATUS_SECONDS = math.max(3 * 60 * 60, numberSetting("GPINATA_WEBHOOK_STATUS_SECONDS", 3 * 60 * 60)),
            WEBHOOK_FARM_LOSS_DELAY = math.max(30, numberSetting("GPINATA_WEBHOOK_FARM_LOSS_DELAY", 60)),
            WEBHOOK_ALERT_REJECT_STREAK = math.max(2, math.floor(numberSetting("GPINATA_WEBHOOK_ALERT_REJECT_STREAK", 3))),
            TARGET_ZONE = tonumber(env.GZONE_TO) or 39,
            FARM_WAIT_TIMEOUT = math.max(60, numberSetting("GPINATA_FARM_WAIT_TIMEOUT", 600)),
            ...
            CIRCUIT_BREAKER_AFTER = math.max(2, math.floor(numberSetting("GPINATA_CIRCUIT_BREAKER_AFTER", 3))),
            CIRCUIT_BREAKER_INITIAL_DELAY = math.max(10, numberSetting("GPINATA_CIRCUIT_BREAKER_INITIAL_DELAY", 30)),
            CIRCUIT_BREAKER_MAX_DELAY = math.max(30, numberSetting("GPINATA_CIRCUIT_BREAKER_MAX_DELAY", 300)),
            ZONE_CIRCUIT_WINDOW_SECONDS = math.max(1800, numberSetting("GPINATA_ZONE_CIRCUIT_WINDOW_SECONDS", 7200)),
            ZONE_CIRCUITS_TO_UNHEALTHY = math.max(2, math.floor(numberSetting("GPINATA_ZONE_CIRCUITS_TO_UNHEALTHY", 2))),
            ZONE_UNHEALTHY_HOLD = math.max(1800, numberSetting("GPINATA_ZONE_UNHEALTHY_HOLD", 3600)),
            PACING_JITTER_MAX = math.max(0, math.min(1, numberSetting("GPINATA_PACING_JITTER_MAX", 0.15))),
            CALIBRATION = env.GPINATA_CALIBRATION ~= false,
            CALIBRATION_PERSIST = env.GPINATA_CALIBRATION_PERSIST ~= false,
            CALIBRATION_MIN_CYCLES = math.max(300, math.floor(numberSetting("GPINATA_CALIBRATION_MIN_CYCLES", 300))),
            CALIBRATION_MIN_SECONDS = math.max(2700, numberSetting("GPINATA_CALIBRATION_MIN_SECONDS", 2700)),
            CALIBRATION_MAX_REJECT_RATE = math.min(0.015, math.max(0, numberSetting("GPINATA_CALIBRATION_MAX_REJECT_RATE", 0.015))),
            AUTO_REMOTE = env.GPINATA_AUTO_REMOTE ~= false,
            AUTO_REMOTE_RESOLVE_TIMEOUT = math.max(5, numberSetting("GPINATA_AUTO_REMOTE_RESOLVE_TIMEOUT", 30)),
            REMOTE_TIMEOUT = math.max(3, numberSetting("GPINATA_REMOTE_TIMEOUT", 8)),
            REMOTE_ERROR_RESTART_AFTER = 5,
            VERBOSE = env.GPINATA_VERBOSE == true,
        }

        SETTINGS.ADAPTIVE_MAX_INTERVAL = math.max(SETTINGS.ADAPTIVE_MIN_INTERVAL, SETTINGS.ADAPTIVE_MAX_INTERVAL)
        SETTINGS.CIRCUIT_BREAKER_MAX_DELAY = math.max(SETTINGS.CIRCUIT_BREAKER_INITIAL_DELAY, SETTINGS.CIRCUIT_BREAKER_MAX_DELAY)

        local currentInterval = math.max(SETTINGS.ADAPTIVE_MIN_INTERVAL, math.min(SETTINGS.ADAPTIVE_MAX_INTERVAL, SETTINGS.INTERVAL))

        local function applyFpsCap(fps)
            if type(setfpscap) ~= "function" then return end
            local ok, err = pcall(setfpscap, fps or SETTINGS.FPS)
            if not ok then warn("[Runtime] FPS cap failed: " .. tostring(err)) end
        end
        ...
        local calibrationFileName = string.format("mini_pinata_calibration_%s_%s_%s.json", tostring(LocalPlayer and LocalPlayer.UserId or 0), tostring(game.PlaceId), tostring(SETTINGS.TARGET_ZONE))
        local calibrationProfile = {
            schema = 2, userId = LocalPlayer and LocalPlayer.UserId or 0, placeId = game.PlaceId, placeVersion = game.PlaceVersion, zone = SETTINGS.TARGET_ZONE,
            recommendedInterval = currentInterval, qualifiedCycles = 0, qualifiedSeconds = 0, sessions = 1, updatedAt = 0, nextRecoveryAllowedAt = 0,
            recentCircuitCount = 0, lastCircuitAt = 0, zoneUnhealthyUntil = 0, engineBuild = ENGINE_BUILD,
        }
        local calibrationPersistent = SETTINGS.CALIBRATION and SETTINGS.CALIBRATION_PERSIST and readCalibrationFile ~= nil and writeCalibrationFile ~= nil

        local function loadCalibrationProfile()
            if not calibrationPersistent then
                dashboard.calibration = SETTINGS.CALIBRATION and "Session calibration" or "Calibration disabled"
                return
            end
            ...
            local profileMatches = tonumber(decoded.userId) == calibrationProfile.userId and tonumber(decoded.placeId) == calibrationProfile.placeId and tonumber(decoded.zone) == calibrationProfile.zone
            ...
            calibrationProfile.recommendedInterval = math.max(SETTINGS.ADAPTIVE_MIN_INTERVAL, math.min(SETTINGS.ADAPTIVE_MAX_INTERVAL, recommended))
            calibrationProfile.sessions = math.max(0, tonumber(calibrationProfile.sessions) or 0) + 1
            calibrationProfile.nextRecoveryAllowedAt = math.max(0, tonumber(calibrationProfile.nextRecoveryAllowedAt) or 0)
            calibrationProfile.recentCircuitCount = math.max(0, math.floor(tonumber(calibrationProfile.recentCircuitCount) or 0))
            calibrationProfile.lastCircuitAt = math.max(0, tonumber(calibrationProfile.lastCircuitAt) or 0)
            calibrationProfile.zoneUnhealthyUntil = math.max(0, tonumber(calibrationProfile.zoneUnhealthyUntil) or 0)
            ...
            currentInterval = math.max(currentInterval, calibrationProfile.recommendedInterval)
            dashboard.calibration = string.format(not versionMatches and "Game update • validating %.2fs" or calibrationProfile.provisional and "Remembered protective %.2fs" or "Remembered %.2fs", calibrationProfile.recommendedInterval)
        end

        local function saveCalibrationProfile()
            if not calibrationPersistent then return false end
            calibrationProfile.schema = 2
            ...
            local encodeOk, encoded = pcall(HttpService.JSONEncode, HttpService, calibrationProfile)
            if not encodeOk then return false end
            local writeOk = pcall(writeCalibrationFile, calibrationFileName, encoded)
            return writeOk
        end

        local function persistRecoveryLock(seconds, reason)
            local lockUntil = os.time() + math.ceil(math.max(0, seconds or 0))
            calibrationProfile.nextRecoveryAllowedAt = math.max(tonumber(calibrationProfile.nextRecoveryAllowedAt) or 0, lockUntil)
        ...
        local function recordZoneCircuit()
            ...
            if previousCircuitAt <= 0 or nowEpoch - previousCircuitAt > SETTINGS.ZONE_CIRCUIT_WINDOW_SECONDS then
                calibrationProfile.recentCircuitCount = 1
            else
                calibrationProfile.recentCircuitCount = math.max(0, math.floor(tonumber(calibrationProfile.recentCircuitCount) or 0)) + 1
            end
            calibrationProfile.lastCircuitAt = nowEpoch
            local markedUnhealthy = calibrationProfile.recentCircuitCount >= SETTINGS.ZONE_CIRCUITS_TO_UNHEALTHY
            if markedUnhealthy then
                calibrationProfile.zoneUnhealthyUntil = math.max(tonumber(calibrationProfile.zoneUnhealthyUntil) or 0, nowEpoch + SETTINGS.ZONE_UNHEALTHY_HOLD)
                calibrationProfile.nextRecoveryAllowedAt = math.max(tonumber(calibrationProfile.nextRecoveryAllowedAt) or 0, calibrationProfile.zoneUnhealthyUntil)
        ...
        local function zoneHealthRemaining()
            return math.max(0, (tonumber(calibrationProfile.zoneUnhealthyUntil) or 0) - os.time())
        end

        loadCalibrationProfile()

        if os.time() - (tonumber(calibrationProfile.lastCircuitAt) or 0) > SETTINGS.ZONE_CIRCUIT_WINDOW_SECONDS then
        ...
        local function remoteAliases()
            local aliases = {
                tostring(env.GPINATA_REMOTE_NAME or "MiniPinata_Consume"), "MiniPinata_Consume", "MiniPinataConsume", "Mini Pinata Consume",
            }
        ...
        local function endpointValid(endpoint)
            ...
            if endpoint.kind == "instance" then return endpoint.remote and endpoint.remote.Parent and endpoint.remote:IsA("RemoteFunction") end
            return endpoint.kind == "network-wrapper" and type(endpoint.invoke) == "function"
        end

        local function announceResolvedEndpoint(endpoint)
            if not endpointValid(endpoint) then return end
            dashboard.remote = endpoint.display
            if endpoint.label == lastResolvedEndpointLabel then return end
        ...
        local function findDirectEndpoint(network, aliases)
            ...
            for index, alias in ipairs(aliases) do
                ...
                if candidate and candidate:IsA("RemoteFunction") then
                    return {
                        kind = "instance", remote = candidate, label = candidate:GetFullName(),
                        display = index == 1 and "Known remote" or "Alias remote", strategy = index == 1 and "Known route" or "Configured alias",
                    }
                end
            end
            if not SETTINGS.AUTO_REMOTE then return nil end
            for _, candidate in ipairs(network:GetDescendants()) do
                if candidate:IsA("RemoteFunction") then
                    ...
                    local mentionsMiniPinata = string.find(normalized, "minipinata", 1, true) ~= nil
                    local looksConsumptive = string.find(normalized, "consume", 1, true) ~= nil or string.find(normalized, "activate", 1, true) ~= nil or string.find(normalized, "use", 1, true) ~= nil
                    if mentionsMiniPinata and looksConsumptive then
                        return {
                            kind = "instance", remote = candidate, label = candidate:GetFullName(),
                            display = "Discovered remote", strategy = "Signature scan",
                        }
                    end
                end
            end
        ...
        local function findNetworkWrapperEndpoint(aliases)
            ...
            if not requireOk or type(module) ~= "table" then return nil end
            local invoke = module.Invoke or module.invoke
            if type(invoke) ~= "function" then return nil end
            return {
                kind = "network-wrapper", invoke = invoke, action = aliases[1],
                label = moduleScript:GetFullName() .. " → " .. aliases[1], display = "Client network wrapper", strategy = "PS99 client network",
            }
        end

        local function getConsumeEndpoint()
            heartbeat("remote resolution")
            if endpointValid(ConsumeEndpoint) then return ConsumeEndpoint end
            ConsumeEndpoint = nil
            ...
            repeat
                ...
                if not endpoint then endpoint = findNetworkWrapperEndpoint(aliases) end
                if endpoint then
        ...
        local function invokeConsumeWithTimeout(endpoint, uid)
            ...
            local invokeThread = task.spawn(function()
                local ok, result = pcall(function()
                    if endpoint.kind == "instance" then return endpoint.remote:InvokeServer(uid) end
            ...
            while not completed and not env.STOP_MINI_PINATA_FAST_PLACER and os.clock() < deadline do
                heartbeat("remote response wait")
                task.wait(0.1)
            end
            if completed then return callOk, callResult, false end
            local cancelled = cancelManagedThread(invokeThread)
            if not cancelled and completed then return callOk, callResult, false end
            if not cancelled then error("[Runtime] Timed-out remote thread could not be cancelled.") end
            if env.STOP_MINI_PINATA_FAST_PLACER then return false, "engine stopped during remote call", false end
            return false, string.format("remote timed out after %.1fs", SETTINGS.REMOTE_TIMEOUT), true
        end

        local function getMapCmds()
            if MapCmds then return MapCmds end
            ...
            if ok and type(module) == "table" then MapCmds = module end
            return MapCmds
        end
        ...
        local function isInsideFarmArea()
            ...
            if type(module) ~= "table" or type(module.IsInDottedBox) ~= "function" then return false end
        ...
        local function waitForFarmArea()
            ...
            print(string.format("[Runtime] Waiting for GScript farming area for target zone %d.", SETTINGS.TARGET_ZONE))
            while not env.STOP_MINI_PINATA_FAST_PLACER do
                heartbeat("waiting for stable farm area")
                if isRecovery and not lossAlerted and os.clock() - waitStartedAt >= SETTINGS.WEBHOOK_FARM_LOSS_DELAY then
                    lossAlerted = true
                    sendWebhook({
                        title = "Farm Area Lost", description = "Placement has remained paused long enough to require attention. The engine is waiting for GScript to restore a stable farming position.",
                        color = 16753920, targetZone = SETTINGS.TARGET_ZONE,
                        fields = {
                ...
                if os.clock() - waitStartedAt >= SETTINGS.FARM_WAIT_TIMEOUT then
                    error(string.format("[Runtime] Farming area was not ready after %.0f seconds.", SETTINGS.FARM_WAIT_TIMEOUT))
                end
                local root = getCharacterRoot()
                if root and isInsideFarmArea() then
                    local position = root.Position
                    if not stablePosition or (position - stablePosition).Magnitude > SETTINGS.POSITION_RADIUS then
                        stablePosition = position
                        stableSince = os.clock()
                    end
                    local positionReady = stableSince and os.clock() - stableSince >= SETTINGS.STABLE_WAIT
                    ...
                    if positionReady and startupReady then
                        ...
                        if lossAlerted then
                            sendWebhook({
                                title = "Farm Area Recovered", description = "GScript returned the account to a stable farming area. Placement has resumed.",
                                color = 5763719, targetZone = SETTINGS.TARGET_ZONE,
                                fields = {
        ...
        local function waitForAccountPhase()
            if not SETTINGS.STAGGER then return true end
            local ok, serverNow = pcall(function() return workspace:GetServerTimeNow() end)
            if not ok or type(serverNow) ~= "number" then return true end
            ...
            if delay < 0.05 then return true end
            if SETTINGS.VERBOSE then print(string.format("[Runtime] Account stagger %.2fs.", delay)) end
        ...
        if not initialUid then
            if inventoryReady then
                ...
                dashboard.pinatas = 0
                sendWebhook({
                    title = "No Mini Pinatas Available", description = "The engine stopped cleanly because this account had no Mini Pinatas when inventory became available.",
                    color = 16753920, targetZone = SETTINGS.TARGET_ZONE,
                    fields = { webhookField("Inventory Remaining", "0", true), webhookField("Placement State", "Stopped", true) },
                })
                return "no_items"
            end
        ...
        if not initialUid then
            if inventoryReady then
                ...
                dashboard.pinatas = 0
                sendWebhook({
                    title = "No Mini Pinatas Available", description = "The engine stopped cleanly because the inventory contained no Mini Pinatas after farming began.",
                    color = 16753920, targetZone = SETTINGS.TARGET_ZONE,
                    fields = { webhookField("Inventory Remaining", "0", true), webhookField("Placement State", "Stopped", true) },
                })
                return "no_items"
        ...
        if not getConsumeEndpoint() then error("[Runtime] Consume endpoint could not be resolved.") end
        ...
        local function resetCalibrationSegment(reason)
            ...
            if SETTINGS.VERBOSE and reason then print("[Runtime] Calibration reset: " .. tostring(reason)) end
        end
        ...
        local function publishTelemetry()
            ...
            pcall(telemetryRequest, {
                Url = telemetryEndpoint, Method = "POST",
                Headers = { ["Authorization"] = "Bearer " .. telemetryWriteKey, ["Content-Type"] = "application/json" },
                Body = body,
            })
        end
        ...
        local function applyAdaptiveIntervalChange(requestedInterval, adjustmentReason, evidence, holdSeconds)
            ...
            sendWebhook({
                ...
                color = slowedDown and 16753920 or 5763719, targetZone = SETTINGS.TARGET_ZONE,
                fields = {
        ...
        local function updateAdaptiveRate(cycleConfirmed, cycleRejected, usedSecondRetry, recoveredFailureStreak, recoveredFromCircuit)
            ...
            local evidence = {
                windowCycles = adaptiveWindowCycles, windowRejected = adaptiveWindowRejected, windowFailed = 0,
                historyCycles = historyCycles, historyRejectRate = historyRejectRate,
            }
        ...
        local function alertOnRejectionStreak()
            if rejectedStreak >= SETTINGS.CIRCUIT_BREAKER_AFTER and not circuitBreakerActive then
                ...
                sendWebhook({
                    ...
                    color = 15548997, targetZone = SETTINGS.TARGET_ZONE, critical = true,
                    fields = {
        ...
        while not env.STOP_MINI_PINATA_FAST_PLACER do
            ...
            for retryIndex = 0, cycleRetryLimit do
                ...
                elseif retryIndex < cycleRetryLimit then
                    ...
                    local dynamicRetryDelay = math.max(0.1, 6.15 - timeSinceSuccess)
    ...
    while not env.STOP_MINI_PINATA_FAST_PLACER do
        ...
        sendWebhook({
            ...
            color = 15548997, targetZone = tonumber(env.GZONE_TO) or 39, critical = true,
            fields = {

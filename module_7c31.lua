...
task.spawn(function()
    local success, runtimeError = xpcall(function()
        ...
        local SETTINGS = {
            INTERVAL = math.max(
                1.5,
                numberSetting("GPINATA_INTERVAL", 6)
            ),

            CONFIRM_WAIT = math.max(
                0.25,
                numberSetting("GPINATA_CONFIRM_WAIT", 1.25)
            ),

            RETRY_DELAY = math.max(
                0.25,
                numberSetting("GPINATA_RETRY_DELAY", 0.75)
            ),

            MAX_RETRIES = math.max(
                0,
                math.floor(
                    numberSetting("GPINATA_MAX_RETRIES", 3)
                )
            ),

            STARTUP_WAIT = math.max(
                0,
                numberSetting("GPINATA_STARTUP_WAIT", 90)
            ),

            STABLE_WAIT = math.max(
                0,
                numberSetting("GPINATA_STABLE_WAIT", 15)
            ),

            FPS = math.max(
                1,
                numberSetting("GPINATA_FPS", 10)
            ),

            STATUS_EVERY = math.max(
                1,
                math.floor(
                    numberSetting("GPINATA_STATUS_EVERY", 10)
                )
            ),
        ...
        local function waitForFarmArea()
            ...
            print(string.format(
                "[Runtime] Waiting for GScript farming area "
                    .. "for target zone %d.",
        ...
        local function waitForInventory()
            local deadline =
                os.clock()
                + SETTINGS.INVENTORY_TIMEOUT

            repeat
                local uid, total, ready =
                    findItemStack()
        ...
        local function recordConfirmation(
            amountUsed,
            totalAfter
        )
            local firstConfirmation =
                confirmed == 0
        ...
        while not env.STOP_MINI_PINATA_FAST_PLACER do
            if not getCharacterRoot()
                or not isInsideFarmArea()
            then
                if not waitForFarmArea() then
                    break
                end
            ...
            for retryIndex = 0, SETTINGS.MAX_RETRIES do
                ...
                local uid, totalBefore, ready =
                    findItemStack()

                if not ready then
                    task.wait(0.5)

                    uid, totalBefore, ready =
                        findItemStack()
                end

                if ready and not uid then
                    print(
                        "[Runtime] No items remain; stopping."
                    )
                ...
                then
                    ...
                    if lateTotal
                        and totalBefore > 0
                        and lateTotal < totalBefore
                    then
                        recordConfirmation(
                            totalBefore - lateTotal,
                            lateTotal
                        )
    ...
    if not success then
        warn(
            "[Runtime] Engine stopped with error: "
